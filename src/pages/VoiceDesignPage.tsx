...
/**
 * ✅ TS(BlobPart) 타입 충돌 해결:
 * Uint8Array<ArrayBufferLike> 를 Blob에 직접 넣지 않고,
 * 명시적으로 ArrayBuffer를 만들어 BlobPart로 전달.
 */
function base64ToObjectUrl(b64: string, mediaType: string): string {
  ...
  for (let i: number = 0; i < len; i++) view[i] = bin.charCodeAt(i);

  const blob: Blob = new Blob([ab], { type: mediaType || "audio/mpeg" });
  return URL.createObjectURL(blob);
}

function ensureMinChars(input: string, min: number): string {
  const base: string = input ?? "";
  if (base.length >= min) return base;

  const padChunk: string =
    " 추가로 불편하신 점을 자세히 말씀해 주시면 더 정확히 확인해 빠르게 도와드리겠습니다. 담당 부서에 즉시 전달해 진행 상황도 함께 안내드릴게요.";
  let out: string = base.trim().length ? base : "안녕하세요. 문의 주셔서 감사합니다.";
...
export function VoiceDesignPage(props: {
  ...
  // ===== Voice Tuning (existing voices) =====
  const [tuneText, setTuneText] = useState<string>("안녕하세요. 톤과 발화 스타일을 튜닝 중입니다.");
  const [tuneAudio, setTuneAudio] = useState<string>("");
  const tuneAudioRef = useRef<HTMLAudioElement | null>(null);
  ...
  const selected = useMemo<ElevenVoice | undefined>(() => voices.find((v: ElevenVoice) => v.voice_id === voiceId), [
    voices,
    voiceId,
  ]);
  ...
  async function genTuningPreview(): Promise<void> {
    const text: string = tuneText.trim();
    if (!voiceId || !text) return;
  ...
  // ===== Voice Design (prompt -> previews -> create) =====
  const [desc, setDesc] = useState<string>("부드럽고 차분한 톤, 또렷한 발음, 과장되지 않은 감정 표현");

  const [previewText, setPreviewText] = useState<string>(
    "안녕하세요. 문의 주셔서 감사합니다. 지금 상황을 빠르게 확인하고, 가능한 해결 방법을 정리해 단계별로 안내드리겠습니다. 불편을 드려 진심으로 죄송합니다. 추가로 필요한 정보가 있으면 바로 요청드릴게요."
  );

  const [autoGenText, setAutoGenText] = useState<boolean>(false);
  ...
  const [newVoiceName, setNewVoiceName] = useState<string>("My ToneShift Voice");
  const [newVoiceDesc, setNewVoiceDesc] = useState<string>("ToneShift에서 생성한 보이스");

  const previewLen: number = (previewText ?? "").length;
  const previewLenTrim: number = (previewText ?? "").trim().length;
  ...
  async function generatePreviews(): Promise<void> {
    const voiceDescription: string = desc.trim();
    const t: string = previewText.trim();

    if (voiceDescription.length < 20) {
      setDesignStatus("error");
      setDesignError("voice description이 너무 짧아요.");
      setVoiceLabStatus("error");
      setVoiceLabError("voice description이 너무 짧아요.");
      return;
    }
    ...
    if (!autoGenText) {
      ...
      if (t.length < MIN_PREVIEW_TEXT_LEN) {
        const msg: string = `preview text는 최소 ${MIN_PREVIEW_TEXT_LEN}자 필요합니다. (현재 ${t.length}자)`;
        setDesignStatus("error");
        setDesignError(msg);
        setVoiceLabStatus("error");
        setVoiceLabError(msg);
    ...
    try {
      ...
      const out: { previews: VoiceDesignPreview[]; text: string } = await createVoiceDesignPreviews({
        voiceDescription,
        text: autoGenText ? undefined : t,
        autoGenerateText: autoGenText,
        quality,
      ...
      const items: PreviewItem[] = (out.previews ?? []).map((p: VoiceDesignPreview) => {
        const url: string = base64ToObjectUrl(p.audio_base_64, p.media_type);
  ...
  async function createVoice(): Promise<void> {
    const picked: string = pickedId.trim();
    if (!picked) return;

    const name: string = newVoiceName.trim();
    const description: string = newVoiceDesc.trim();
    ...
    try {
      const created: ElevenVoice = await createVoiceFromDesign({
        voiceName: name,
        voiceDescription: description,
        generatedVoiceId: picked,
      });
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <section className="ts-card p-5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <RangeRow
            label="stability"
            value={tuneSettings.stability}
            min={0}
            max={1}
            step={0.01}
            onChange={(v: number) => setTuneSettings((prev: ElevenVoiceSettings) => ({ ...prev, stability: v }))}
          />
          <RangeRow
            label="similarity_boost"
            value={tuneSettings.similarity_boost}
            ...
            max={1}
            step={0.01}
            onChange={(v: number) => setTuneSettings((prev: ElevenVoiceSettings) => ({ ...prev, similarity_boost: v }))}
          />
          <RangeRow
            label="style"
            value={tuneSettings.style ?? 0}
            min={0}
            max={1}
            step={0.01}
            onChange={(v: number) => setTuneSettings((prev: ElevenVoiceSettings) => ({ ...prev, style: v }))}
          />
          <RangeRow
            label="speed"
            value={tuneSettings.speed ?? 1}
            min={0.7}
            max={1.2}
            step={0.01}
            onChange={(v: number) => setTuneSettings((prev: ElevenVoiceSettings) => ({ ...prev, speed: v }))}
          />
        </div>
        ...
        <div className="mt-3">
          <label className="ts-switch">
            <input
              ...
              checked={!!tuneSettings.use_speaker_boost}
              onChange={(e) =>
                setTuneSettings((prev: ElevenVoiceSettings) => ({ ...prev, use_speaker_boost: e.target.checked }))
              }
            />
            <span className="ts-switchTrack">
              <span className="ts-switchThumb" />
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <button className="ts-btn ts-btn-accent" onClick={() => void genTuningPreview()} disabled={tuneStatus === "loading"}>
            {tuneStatus === "loading" ? <span className="ts-spinner" /> : "🔊"} 미리듣기 생성
          </button>
          <button
            className="ts-btn ts-btn-ghost"
      ...
      <section className="ts-card p-5">
        <div className="ts-h">
          ...
          <span className="ts-pill">min text: {MIN_PREVIEW_TEXT_LEN}자</span>
        </div>
        ...
        <div className="mt-4 grid gap-3">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span
              className="ts-pill"
              style={
                !autoGenText && previewLenTrim > 0 && previewLenTrim < MIN_PREVIEW_TEXT_LEN
                  ? { borderColor: "rgba(255,77,109,.45)", color: "rgba(255,122,144,.95)" }
                  : undefined
              }
              title={`raw: ${previewLen}, trimmed: ${previewLenTrim}`}
            >
              text length: {previewLenTrim}/{MIN_PREVIEW_TEXT_LEN}
              {autoGenText ? " (auto)" : ""}
            </span>
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <button
                ...
                className="ts-btn ts-btn-ghost"
                disabled={autoGenText || previewLenTrim >= MIN_PREVIEW_TEXT_LEN}
                onClick={() => setPreviewText((prev: string) => ensureMinChars((prev ?? "").trim(), MIN_PREVIEW_TEXT_LEN))}
                title="현재 텍스트 뒤에 자연스럽게 문장을 덧붙여 최소 100자를 맞춥니다."
              >
                ✍️ 100자 자동채우기
              </button>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            <button
              ...
              disabled={designStatus === "loading" || (!autoGenText && previewLenTrim < MIN_PREVIEW_TEXT_LEN)}
            >
              {designStatus === "loading" ? <span className="ts-spinner" /> : "✨"} 프리뷰 생성
            </button>
            <button className="ts-btn ts-btn-ghost" onClick={clearPreviews}>
            ...
            {!autoGenText && previewLenTrim < MIN_PREVIEW_TEXT_LEN ? (
              <span className="ts-pill" style={{ borderColor: "rgba(255,77,109,.45)", color: "rgba(255,122,144,.95)" }}>
                텍스트 {MIN_PREVIEW_TEXT_LEN}자 이상 필요
              </span>
            ) : null}
          </div>
          ...
          <div className="mt-2">
            {previews.length ? (
              <div className="grid gap-3 sm:grid-cols-3">
                {previews.map((p: PreviewItem) => {
                  const picked: boolean = p.generated_voice_id === pickedId;
                  return (
                    <button
                      key={p.generated_voice_id}
                      type="button"
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            <button
              ...
              disabled={!pickedId || designStatus === "loading"}
            >
              {designStatus === "loading" ? <span className="ts-spinner" /> : "📦"} 선택 프리뷰로 Voice 저장
            </button>
            <span className="ts-pill">picked: {pickedId ? `${pickedId.slice(0, 12)}…` : "-"}</span>
