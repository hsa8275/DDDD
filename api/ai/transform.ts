...
async function readBody(req: NodeReq): Promise<string> {
  ...
  return await new Promise<string>((resolve, reject) => {
    ...
    req.on("end", () => {
      const buf: Uint8Array = chunks.length === 1 ? chunks[0] : Buffer.concat(chunks);
...
export default async function handler(req: NodeReq, res: NodeRes): Promise<void> {
  const method: string = String(req.method ?? "POST").toUpperCase();
  ...
  try {
    ...
    if (!isRecord(data)) {
      sendJson(res, 400, { ok: false, error: "invalid_body", message: "JSON object body required" });
      return;
    }

    const upstream: Response = await fetch("http://34.64.208.249:80/ai/transform", {
      method: "POST",
